...
class FizzBuzzSpec extends Specification {
  "if a number is divisible by three you get Fizz and five you get Buzz both you get FizzBuzz" should {
    ...
    "for 1..2" in {
      FizzBuzzOne(1).take(2).toList.map{_.value} must beEqualTo(
        List("1", "2")
      )
    }
    "for 1..10" in {
      FizzBuzzOne(1).take(10).toList.map {_.value} must beEqualTo(
        List(
    ...
    "for 50..60" in {
      FizzBuzzOne(50).take(10).toList.map {_.value} must beEqualTo(
